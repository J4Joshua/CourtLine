...
// ⚙️ CONFIG — legacy WebSocket video/photo path (the glasses-streaming template's
// transport). The CourtLine agent integration uses WebRTC for audio (see
// WebRTCAudioSpike.swift); video-over-WebRTC / wiring frames into the agent's
// vision endpoint is future work. Left as a placeholder for now.
...
final class StreamSessionViewModel {
  ...
  /// PROTOCOL.md §2.6 de-risking spike: a bidirectional WebRTC audio leg
  /// (direct to our own server via Pipecat SmallWebRTC) over HFP, run
  /// *alongside* the DAT camera stream to prove the two coexist before
  /// committing to the full WebRTC migration. Not part of the production
  /// WebSocket path — toggled from the DEBUG controls only.
  let webrtcSpike = WebRTCAudioSpike()
  ...
  // MARK: - WebRTC audio spike (PROTOCOL.md §2.6)

  /// Bring up the WebRTC audio leg over HFP while the DAT camera stream keeps
  /// running. Tears down the hand-rolled WebSocket audio path first so the
  /// SmallWebRTC transport has sole ownership of `AVAudioSession` — the camera
  /// stream (BT Classic) is left untouched, which is the coexistence we test.
  func startWebRTCAudioSpike() async {
    audioPublisher.stop()
    agentPlayer.stop()
    await webrtcSpike.start()
  }

  func stopWebRTCAudioSpike() async {
    await webrtcSpike.stop()
  }

  var isWebRTCSpikeActive: Bool {
    webrtcSpike.state == .joining || webrtcSpike.state == .joined
  }
